...
ForceFeedbackController::ForceFeedbackController(ros::NodeHandle &nh)
    ...
    _subscribers[ctrlCmdTopic] = _nh.subscribe<sensor_msgs::Joy>(
        ctrlCmdTopic, 5, [this](const sensor_msgs::JoyConstPtr &msg) {
            _operatorSWA = -msg->axes[joystick::AxesPos::STEERING]*7.8538;
            // _operatorSWA = msg->axes[joystick::AxesPos::STEERING];
...
void ForceFeedbackController::run() {
    ...
    while (ros::ok()) {
        ros::spinOnce();
        // std::cout<<_operatorSWA<<std::endl;
        if (_vehicleDataMsg) {
            double currentWheelPosition =_operatorSWA;
            ...
            // std::cout<<"desired weheel"<<desiredWheelPosition<<std::endl;
            // std::cout<<"ff value"<<ffValue<<std::endl;

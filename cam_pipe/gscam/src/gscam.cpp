...
namespace gscam {
  ...
  bool GSCam::init_stream()
  {
    ...
    g_object_set(G_OBJECT(sink_),
                 "emit-signals", FALSE,
                 "sync", (sync_sink_) ? TRUE : FALSE,
                 "max-buffers", 1,      // 최신 프레임만
                 "drop", TRUE,          // 초과 시 드롭
                 NULL);
  ...
  void GSCam::publish_stream()
  {
    ...
    while(ros::ok()) 
    {
      ...
      GstBuffer* buf = gst_sample_get_buffer(sample);  // 소유권은 sample이 가짐
      if (!buf) {
        ROS_ERROR("Null GstBuffer from sample.");
        gst_sample_unref(sample);
        break;
      }
      GstMapInfo map;
      if (!gst_buffer_map(buf, &map, GST_MAP_READ)) {
        ROS_ERROR("Failed to map GstBuffer.");
        gst_sample_unref(sample);
        break;
      }

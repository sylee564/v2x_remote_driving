...
// CONSTRUCTORS
tcp_client_t::tcp_client_t(boost::asio::io_context& io_context, uint32_t id, std::function<void(boost::asio::ip::tcp::socket*)> connection_callback)
    : m_id(id)
{
    // Initialize socket.
    tcp_client_t::m_socket = new boost::asio::ip::tcp::socket(io_context);

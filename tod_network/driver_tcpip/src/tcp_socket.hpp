...
private:
    ...
    // std::unique_ptr<Crc16Checker> crc16_checker;
    uint16_t seq{0};
    bool isVehicle{false};
    bool psid_register_flag{false};
    ...
    template <class Msg> 
    void tx_callback(const Msg& message);
    void callback_status(const tod_msgs::Status::ConstPtr& status);
    void handle_send(const boost::system::error_code& error, std::size_t bytes_transferred);
    void AddExtStatusData(TLVC_Overall *p_overall, int tx_rx);
    void psid_result(uint8_t *msg, int len);
    ...
    ros::Subscriber m_subscriber_status;
    ...
    void stop_ros();
    uint16_t CalcCRC16(const unsigned char * pucaData, uint16_t usLenData);
